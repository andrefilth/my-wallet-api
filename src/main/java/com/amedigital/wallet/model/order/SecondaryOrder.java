...
public abstract class SecondaryOrder extends Order {

    protected SecondaryOrder(Builder builder) {
        this.id = builder.id;
    ...
    public OrderLevel getOrderLevel() {
        return OrderLevel.SECONDARY;
    }

    @Override
    public abstract SecondaryOrder.Builder copy();

    public abstract static class Builder<B extends Builder, T extends SecondaryOrder> extends Order.Builder<B, T> {
        ...
        public Builder(String referenceOrderUuid, OrderType type) {
            this.type = type;
        ...
        public B setTransactions(List<Transaction> transactions) {
            ...
            setPaymentMethods(Optional.ofNullable(transactions).orElse(Collections.emptyList()).stream().map(Transaction::getPaymentMethod).map(PaymentMethod::name).collect(Collectors.toList()));
            return (B) this;
        }
